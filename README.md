...
```shell
npm pack /path/to/<JAVASCRIPT_CLIENT_DIR> && npm install ./KindeReactNativeOAuth-0.2.1.tgz
```
...
To access the user information, use the `UserApi` class exported from `KindeReactNativeOAuth`, then call the `getUserProfile` method of `UserApi` instance
